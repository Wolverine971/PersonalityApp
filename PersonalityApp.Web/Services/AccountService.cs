...
namespace PersonalityApp.Web.Services
{
    public class AccountService : IAccountService
    {
        ...
        public AccountService(IAuthenticationService authSerice, ICryptographyService cryptographyService, IDataProvider dataProvider)
        {
        ...
        public User ForgotPasswordValidateAccount(string email)
        {
            User singleItem = null;
            _dataProvider.ExecuteCmd("dbo.Person_SelectEmailConfirmationForLogin", inputParamMapper: delegate (SqlParameterCollection paramCollection)
            {
                paramCollection.AddWithValue("@Email", email);
            },
             singleRecordMapper: delegate (IDataReader reader, short set)
             {
                 singleItem = new User();
                 int ord = 0;
                 singleItem.IsEmailConfirmed = reader.GetSafeBool(ord++);

             });
            // If account exists, but email is not confirmed, re-send Email Confirmation
            if (singleItem != null && !singleItem.IsEmailConfirmed)
            {
